...
HELLO_TEXT = {'ru': '''
Привет, я сквозь фронтальную камеру уже вижу, что ты просто чудо)
...
Хочешь разнообразия? Смени язык на "неклассический".
Хочешь сладких апельсинов? Сходи в магазин.
Напомнить адрес студофиса? Задай нам любой вопрос🤪☺️😉
...
INPUT_SUBJ_NAME_SUCCESS = {'ru': 'Предмет добавлен в базу'}
INPUT_FIO = {'ru': 'Введите ФИО преподавателя', 'newru': "Вопрос на пятерку: Как зовут преподавателя? "}
OTZYV = {'ru': 'Оставьте отзыв о преподавателе', 'newru': "Ну и как он в плане учёбы?"}
...
TEACHER_LIST_TITLE = {'ru': 'Выберите преподавателяи', 'newru': "Choose your препод"}
SUBJECT_LIST_TITLE = {'ru': 'Выберите предмет', 'newru': "Choose your предмед "}
...
STRING_POLLS = {
   'ru': [
       ...
       ['Устраивает ли вас то, как преподаватель проводит дисциплину?', ['Да, всё отлично', 'Можно лучше', 'Приемлемо', 'Не устраивает']],
   ...
   'newru': [
       ...
       ['Идеальный формат работы', ['Сколотить дринк тим/попасть в суйсайд сквад', 'Всолянчик', 'Лично мне всё равно']],
