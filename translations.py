...
MENU_HELLO = {'ru': 'Привет', 'newru': "Здарова/Приветики пистолетики/Салют"}
...
Я хочу чтобы ты потрогал(-а) меня везде... Перемой косточки всем преподавателям, пройди все опросы по дисциплинам и помоги улучшить качество твоего образования.
...
WRITE_ONE_MORE = {'ru': 'Написать заново', 'newru': "Ашибся, сейчас всё исправлю"}
...
HOW_REVIEW_TEACHER = {'ru': "Как вы оцените преподавателя? (из 10)", 'newru': "На сколько он(-а) крут(-а) по десятибалльной?"}#надо поменять
...
STRING_POLLS = {
   'ru': [
       ...
       ['Вопрос4', ['Варианты4', 'ответов4', 'ответов1', 'ответов1', 'ответов1', 'ответов1']],
   ],
   'newru': [
       ['Ну и как тебе предмет, сладкий?', ['Апупенный', 'Четвертинка наполовинку', 'Отстой(Креативные технологии)']],
       ['Что можешь сказать по количеству?', ['Тютелька в тютельку', '', 'Слишком много, в меня столько не влезает', 'Ой, а разговоров-то было!', 'Да кому он нужен (Креативные технологии) Казнить, нельзя помиловать', 'Пофек']],
       ['Идеальный формат работы', ['Сколотить дринк тим или надеяться, чтобы не попался суйсайд сквад', 'Всолянчик', 'До звезды']],
       ['Как тебе препод?', ['Лучше только Путин', 'Я бы дал ему пару консультаций', 'Норм', '', 'Я проведу в сто раз лучше']],# заменяем Путiна на что-то
       ['Вопрос4', ['арианты4', 'тветов4', 'ответов1', 'ответов1', 'ответов1', 'ответов1']],
   ],
}
YES = {'ru': 'Да', 'newru': "Угу"}
