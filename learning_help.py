...
def chose_subject(update: Update, context):
    ...
    if data == Answer.POP_SUBJ_OTHER:
        query.edit_message_text(_(tr.POPULAR_SUBJECT_OTHER_PROMPT, context))
        return State.HELP_CHOSE_SUBJECT_OTHER

    context.user_data[SUBJECT] = data
    choose_type_prompt(query.edit_message_text, context)
...
def chose_type(update, context):
    ...
    if data == Answer.TYPE_TOGETHER or data == Answer.IM_STRONK:
        choose_description_prompt(query.edit_message_text, context)
        return State.HELP_MATCHING
    elif data == Answer.IM_WEAK:
        choose_award_prompt(query.edit_message_text, context)
...
def save_description_and_match(update, context):
    context.user_data[DESCRIPTION] = update.message.text

    update.effective_user.send_message("Sorry, this feature"
                                       " is under construction",
                                       reply_markup=get_main_keyboard(context))
    return on_exit(update, context)
...
def get_states():
    return {
        ...
        State.HELP_CHOSE_AWARD: [MessageHandler(Filters.text, chose_award)],
        State.HELP_MATCHING: [
            MessageHandler(Filters.text, save_description_and_match)
        ],
    }
